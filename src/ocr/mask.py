...
def process_pdf_file(input_pdf:str, output_dir:str, model:AutoModel, tokenizer:AutoTokenizer, system_prompt:str, td:TableDetector)->None:
    ...
    dets = td.predict(image=image)
    dets = dets[0]
    x1, y1, x2, y2 = map(int, dets)  # Convert coordinates to integers
...
def main():
    ...
    for filename in os.listdir(args.directory):
        if filename.lower().endswith(".pdf"):
            ...
            process_pdf_file(input_pdf, args.output_dir, model, tokenizer, system_prompt, td)
